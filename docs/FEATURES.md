...
- **What:** While recording, the capture bar now shows an explicit **Stop record** button (in addition to the record-toggle icon) so active recording can be ended immediately. **Why:** clearer stop affordance during Play + record and manual recording flows.
...
- **What:** In Mosaic, capture-bar **Play + record** now detects active **Stitch-in** (Noise/Bleed) and records a stitched reveal replay for the configured Stitch-in duration, then auto-stops recording. **Why:** makes one-click capture of stitch-in animations reliable without requiring manual keyframe setup.
